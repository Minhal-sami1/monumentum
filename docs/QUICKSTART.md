...
```
pip install -e .            # from the repo; or `pip install agentloop` once published
agentloop --version
```
...
This scaffolds `.loop/` (default policy: context L2, capability L1,
...
```
agentloop gate <cs-id>     # prints GATE_APPROVED for a context change with independent evidence
agentloop apply <cs-id>    # snapshots, applies, journals, commits
```
...
`agentloop approve <cs-id> --actor human/you`.

## 6. Audit and undo

```
agentloop log            # who changed what, when, and why
agentloop verify .       # journal chain intact + managed files match
agentloop rollback <cs-id>   # one command back to the exact prior bytes
```

## 7. Claude Code users: install the skill

```
agentloop install-skill
```

This installs the `loop` skill and a `PreToolUse` hook that denies direct
edits to managed files (pointing the agent at `agentloop propose`), plus a
