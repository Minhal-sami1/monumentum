...
### DEC-043: the protocol is renamed `agentloop` → `monumentum`
- **What:** The owner's pre-publication naming step (GOAL F2, §11). Applied consistently: the Python package and CLI (`monumentum`; the design-doc alias `loop` is kept — one line, no known conflict; the old `agentloop` entry point is *dropped* because an unrelated PyPI package of that name exists and a second `agentloop` console script would shadow it), the SDK (`monumentum-sdk`, import `monumentum_sdk`; the class `Loop` is kept as the mechanism noun), the workspace directory (`.loop/` → `.monumentum/`), the spec tag (`loop/v0.1` → `monumentum/v0.1`), spec text, the five schemas, the conformance corpus, scenarios, the adversarial suite, experiments, demo, all docs, and the paper. Lower-case "loop" survives where it names the improvement loop itself; "the Loop standard" became "Monumentum".
- **Why this name:** The live collision check (design-doc §13, §14) ruled out every listed candidate and the working name: Ratchet (HuggingFace ML framework, a CI-security tool), Lamarck (two agent-lesson projects), Cairn (four AI/agent projects, one nearly the same pitch), agentloop (taken in-domain on PyPI and npm), plus later candidates — provenloop, nacre, varve, stele, escapement, phronesis, moraine, isnad — each with an active agent-memory project using the same metaphor, several created the week of the search. `monumentum` was free on PyPI and npm, had 40 trivial repositories and an empty GitHub org, and carries the right meaning: Horace, *Odes* III.30, *exegi monumentum aere perennius* — a *monumentum* (from *monere*, to remind) is a record that outlasts its maker. That is the journal's promise.
- **How the rename was applied to the repository's own loop:** `.loop/` was moved with `git mv`, so every journal byte is unchanged and the chain verifies. The one required edit to `policy.yaml` (spec tag, protected pattern) is human-only under I1; it was executed by the agent at the owner's explicit direction and journaled as `policy_changed` (seq 8) with actor `producer/claude-code` and `ext.policy_change.directed_by = human/minhal` — not as `human/minhal`, per the actor-honesty lesson of DEC-040. Historical ChangeSets keep their original `loop/v0.1` tag and historical journal actors keep `executor/agentloop@0.1.0`; both are immutable records of the version they were made under.
- **The governed surfaces went through the loop, not through sed:** `AGENTS.md` (context, L2) was renamed by ChangeSet `cs-20260905-rnm1` with command-transcript evidence — auto-applied and committed by the executor. `skill/**` (capability, L1) was proposed as `cs-20260905-rnm2` with a *measured* before/after run of `tests/test_hook.py` and is **queued for the owner's approval**; the gate will not pass until then, which is the design working as intended. The first "after" measurement failed because the test still asserted the pre-rename phrase; the test was aligned and the payload re-measured unchanged (8/8), attached as `ev-002` alongside the original `ev-001` so the reviewer sees both.
