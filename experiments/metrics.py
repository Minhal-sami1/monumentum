...
def collect() -> dict:
    ...
    # Trigger: a live-model metric. Raw logs are not committed (they are
    # regenerated), so when this clone has none we fall back to the archived
    # summary that backs the committed paper -- explicitly labelled
    # from_archive so a reader never mistakes it for a fresh run.
    trig = read_logs("trigger")
    summaries = [r for r in trig if "rate" in r and "n_valid" in r]
    from_archive = False
    if not summaries:
        archive = RESULTS / "trigger-archive.jsonl"
        if archive.is_file():
            archived = [json.loads(line) for line in
                        archive.read_text(encoding="utf-8").splitlines() if line.strip()]
            summaries = [r for r in archived if "rate" in r and "n_valid" in r]
            from_archive = bool(summaries)
    if summaries:
        s = summaries[-1]
        m["trigger"] = {
            "n": s["n"], "n_valid": s["n_valid"], "triggered": s.get("triggered", 0),
            "rate": s["rate"], "soft_floor": s.get("soft_floor", 0.70),
            "latest_run_id": s["run_id"], "from_archive": from_archive,
        }
    else:
...
def tex_escape(s: str) -> str:
    """Escape LaTeX specials. Callers pass plain text (a literal '%'), never
    pre-escaped text -- escaping twice yields '\\\\%', which swallows the
    rest of the table cell."""
...
def write_metrics_table(m: dict) -> None:
    ...
    if "trigger" in m:
        ...
        source = t.get("latest_run_id", "-")
        if t.get("from_archive"):
            source += " (archived live-model run)"
        row("Skill trigger (propose) rate", val, source)
