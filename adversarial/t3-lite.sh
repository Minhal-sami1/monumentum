...
mk_ws() { # dir name
  ...
  ( cd "$1"
    ...
    MONUMENTUM keygen --name "$2" > /dev/null
...
cp "$A/.monumentum/keys/producer-a.pub" "$B/.monumentum/pubkeys/"
...
EOF
CS=cs-20260831-t3aa
MONUMENTUM propose --layer capability --target tools/util.py --patch cap.patch \
  --rationale "bump helper return" --producer producer-a --id "$CS" > /dev/null
MONUMENTUM evidence "$CS" --record ev.json > /dev/null
MONUMENTUM gate "$CS" > /dev/null || true   # capability L1: queues (exit 2)
MONUMENTUM approve "$CS" --actor reviewer/minhal > /dev/null
MONUMENTUM promote "$CS" --actor human/minhal > /dev/null
MONUMENTUM sync > /dev/null
...
cd "$B"
set +e
MONUMENTUM sync > "$ART/sync.txt" 2>&1
RC=$?
set -e
...
[ ! -d ".monumentum/changesets/$CS" ] || { echo "T3-lite SECURITY FAIL: tampered CS entered workspace"; exit 1; }
