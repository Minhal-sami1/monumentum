...
Each test performs a real attack against the real `monumentum` binary and
...
| `t4` | Loop self-capture: (a) ChangeSet targets `.monumentum/policy.yaml`; (b) hand-edited journal | Invariant I1 + the `verify` CI backstop | (a) refused; (b) `verify` fails |
