...
"$PY" "$HERE/check_no_hardcoded.py" "$HERE/loop-paper.tex"

# 3. build (tectonic is self-contained; falls back to latexmk/pdflatex)
cd "$HERE"
mkdir -p build
if command -v tectonic >/dev/null 2>&1; then
  tectonic -X compile loop-paper.tex --outdir build --keep-logs
elif command -v latexmk >/dev/null 2>&1; then
  latexmk -pdf -outdir=build loop-paper.tex
elif command -v pdflatex >/dev/null 2>&1; then
  mkdir -p build
  pdflatex -output-directory=build loop-paper.tex
  pdflatex -output-directory=build loop-paper.tex
else
  echo "no LaTeX engine found (tectonic/latexmk/pdflatex)"; exit 1
fi

test -f build/loop-paper.pdf
echo "paper: built paper/build/loop-paper.pdf"
